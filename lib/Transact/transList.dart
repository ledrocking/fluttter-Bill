...
class _TransListState extends State<TransList> {
  ...
  String _periodName =
      DateFormat('MMMM yyy').format(DateTime.parse(DateTime.now().toString()));
            ...
            Expanded(
              child: Container(
                ...
                child: FutureBuilder<List<TransBill>>(
                    future:
                        DatabaseHelper.instance.fetchJoin2(_myDate.toString()),
                    builder:
                        (context, AsyncSnapshot<List<TransBill>> snapshot) {
                                ...
                                children: [
                                  Column(
                                    children: [
                                      ...
                                      Text("$paidItem Bills, Total $paidTotal"),
                                    ],
                                  ),
                                  Column(
                                    children: [
                                      ...
                                      Text(
                                          "${totItem - paidItem} Bills, Total $myDue"),
                                    ],
                                  ),
...
/*                                        Text("(${(DateTime.parse(item.dueDate)).difference(DateTime.now()).inDays} days) "),
                                    ...
                                    trailing: Column(
                                      children: [
                                        IconButton(
                                          icon: getIcon(context, item.status,
                                              item, item.billName),
                                        ),
                                      ],
                                    ),
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () {
          ...
          Navigator.of(context).push(
            MaterialPageRoute(
                builder: (context) => MyForm(title: "Add New Bill")),
          );
        },
        tooltip: 'Add Bill',
  ...
  changePeriod(int source) {
    ...
    setState(() {
      ...
      _periodName =
          DateFormat('MMMM yyy').format(DateTime.parse(_myDate.toString()));
    });
  }
}
...
detailInfo(TransBill item) {
  ...
  String daysLeftShow;
  if (daysLeft > 1) {
    daysLeftShow = "${-daysLeft} days";
  }
  if (daysLeft == 1) {
    daysLeftShow = "${-daysLeft} day";
  }
  if (daysLeft == 0) {
    daysLeftShow = "$daysLeft day";
  }
  ...
  switch (item.status) {
      ...
      return Wrap(
        children: [
          ...
          Text(DateFormat('EEE, MMM d, ' 'yy')
              .format(DateTime.parse(item.payDate))),
          Text(",   "),
          //           Text("\t "),
    ...
    default:
      return Wrap(
        children: [
          Text(
              "Due : ${DateFormat('EEE, MMM d, ' 'yy').format(DateTime.parse(item.dueDate))},  "),
...
getIcon(context, String status, TransBill item, String billName) {
  ...
  switch (status) {
    case "Paid":
      return Icon(Icons.check_circle, color: Color(0xff1AC5A6));
      break;
    case "Partial":
      return IconButton(
          ...
          onPressed: () {
            Navigator.of(context).push(MaterialPageRoute(
                builder: (context) => TransactUpdate(item, billName)));
          });
      break;
